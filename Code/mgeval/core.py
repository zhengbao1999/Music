...
class metrics(object):
    def total_used_pitch(self, feature):
        ...
        used_pitch = 0
        sum_notes = 0
        for instrument in feature['pretty_midi'].instruments:
    ...
    def total_used_note(self, feature, track_num=1):
        ...
        #print(len(feature['pretty_midi'].instruments))
            if len(track) > 0:
                for i in range(0, len(track)):
                    if type(track[i]) == midi.events.NoteOnEvent and track[i].data[1] != 0:
                        used_notes += 1
        return used_notes
    ...
    def total_pitch_class_histogram(self, feature):
        ...
        #piano_roll = feature['pretty_midi'].instruments[1].get_piano_roll(fs=100)
    ...
    def pitch_range(self, feature):
        ...
        piano_roll = feature['pretty_midi'].get_piano_roll(fs=100)
        pitch_index.append(np.where(np.sum(piano_roll, axis=1) > 0))
        #print(len(pitch_index[0]))
        p_range = 0
        for p in pitch_index:
            if len(p[0]) > 0:
                p_range = max(p_range, np.max(p) - np.min(p))
        return p_range

    def find_melody_instr_index(self, ref_midi):
        #print(ref_midi.instruments)
        if len(ref_midi.instruments) == 1:
            return 0
        track_note = []
        ...
        if len(track_note) == 0:
            for instrument in ref_midi.instruments:
                ...
                for note in instrument.notes:
                    notes.append(note.pitch)
                track_note.append(notes)
        length_track_note = [len(track_note[i]) for i in range(0, len(track_note))]
        mean_track_note = [sum(track_note[i])/len(track_note[i]) for i in range(0, len(track_note))]
        #metric
        metric_track_note = [pow(sum(track_note[i]), 2)/len(track_note[i]) for i in range(0, len(track_note))]
        
        melody_instr_index = metric_track_note.index(max(metric_track_note))
        #print(ref_midi.instruments[melody_instr_index].program)
        return melody_instr_index
    ...
    def avg_pitch_shift(self, feature, track_num=1):
        ...
        for track in pattern:
            if len(track) > 0:
                for i in range(0, len(track)):
                    if type(track[i]) == midi.events.NoteOnEvent and track[i].data[1] != 0:
                        if counter >= total_used_note:
                            i = 999999
                        elif counter != 0:
                            d_note += abs(current_note - track[i].data[0])
                            current_note = track[i].data[0]
                            counter += 1
                        else:
                            current_note = track[i].data[0]
                            counter += 1
        
        pitch_shift = d_note/total_used_note         
        return pitch_shift
    ...
    def note_length_hist(self, feature, track_num=1, normalize=True, pause_event=False):
        ...
        ref_midi = feature['pretty_midi']
        if len(ref_midi.instruments) == 1:
            melody_instr_index = 0
        track_note = []
        for instrument in ref_midi.instruments:
            if instrument.is_drum:
                track_note.append([0])
        ...
        track_num = melody_instr_index
        #print(track_num)

        if len(pattern) == 1:
            #print('one instrument')
            if pause_event is False:
                ...
                for i in range(0, len(pattern[0])):
                    if type(pattern[0][i]) == midi.events.TimeSignatureEvent:
                        time_sig = pattern[0][i].data
                        bar_length = time_sig[0] * resolution * 4 / 2**(time_sig[1])
                    elif type(pattern[0][i]) == midi.events.NoteOnEvent and pattern[0][i].data[1] != 0:
                        if 'time_sig' not in locals():  # set default bar length as 4 beat
                            bar_length = 4 * resolution
                            time_sig = [4, 2, 24, 8]
                        unit = bar_length / 96.
                        hist_list = [unit * 96, unit * 48, unit * 24, unit * 12, unit * 6, unit * 72, unit * 36, unit * 18, unit * 9, unit * 32, unit * 16, unit * 8]
                        current_tick = pattern[0][i].tick
                        current_note = pattern[0][i].data[0]
                        # find next note off
                        for j in range(i, len(pattern[0])):
                            if type(pattern[0][j]) == midi.events.NoteOffEvent or (type(pattern[0][j]) == midi.events.NoteOnEvent and pattern[0][j].data[1] == 0):
                                if pattern[0][j].data[0] == current_note:

                                    note_length = pattern[0][j].tick - current_tick
                        ...
                        for j in range(i, len(pattern[0])):
                            ...
                            if type(pattern[0][j]) == midi.events.NoteOffEvent or (type(pattern[0][j]) == midi.events.NoteOnEvent and pattern[0][j].data[1] == 0):
                                if pattern[0][j].data[0] == current_note:
                                    ...
                                    if pattern[0][j].tick == current_tick:
                                        check_previous_off = False

                        # find previous note off/on
                        if check_previous_off is True:
                            for j in range(i - 1, 0, -1):
                                if type(pattern[0][j]) == midi.events.NoteOnEvent and pattern[0][j].data[1] != 0:
                                    break

                                elif type(pattern[0][j]) == midi.events.NoteOffEvent or (type(pattern[0][j]) == midi.events.NoteOnEvent and pattern[0][j].data[1] == 0):

                                    note_length = current_tick - pattern[0][j].tick
                                    distance = np.abs(np.array(hist_list) - note_length)
                                    idx = distance.argmin()
                                    if distance[idx] < tol:
        ...
        if normalize is False:
            ...
            if np.sum(note_length_hist) == 0:
                return note_length_hist*0
            else:
